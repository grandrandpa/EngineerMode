package com.cdtsp.engineermode.view;
...
public class SwitchItem extends RelativeLayout {
    private static final String TAG = "SwitchItem";
    ...
    public SwitchItem(Context context, @Nullable AttributeSet attrs) {
        ...
        //获取属性
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SwitchItem);
        String title = ta.getString(R.styleable.SwitchItem_title);
        String subTitle = ta.getString(R.styleable.SwitchItem_subtitle);
        String statusText = ta.getString(R.styleable.SwitchItem_statusText);
        //String ip = ta.getString(R.styleable.SwitchItem_ip);
        int textSize = ta.getDimensionPixelSize(R.styleable.SwitchItem_textSize, 20);
        int switchDrawableResId = ta.getResourceId(R.styleable.SwitchItem_switchDrawable, -1);
    ...
    public void switchIconStateTo(boolean on) {
        mImgSwitch.setSelected(on);
    }
    ...
    private void addSubTitleView(String subTitle) {
        ...
        LayoutParams paramsTextView = new LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT);
        paramsTextView.addRule(RelativeLayout.BELOW, R.id.title_id);//靠title下摆放
     ...
     * @param text
     */
    private void addIPTextView(String text) {
        mTvIP = new TextView(mContext);
        mTvIP.setText(text);
        mTvIP.setTextSize(30);
        mTvIP.setTextColor(Color.DKGRAY);
        LayoutParams paramsTextView = new LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT);
        paramsTextView.leftMargin = 100;
        paramsTextView.addRule(RelativeLayout.BELOW, R.id.title_id);//靠左摆放

        addView(mTvIP, paramsTextView);
    }
